...
while not gameover:  #Algab while tsükkel, kui ei ole gameover.
    ...
    for i in range(len(coords)): #i saab koordinaadid vastavalt objektide arvule.
        screen.blit(blue, [coords[i][0], coords[i][1], 20, 20]) #Sinine auto liigub juhuslike kordinaatide abil.
        coords[i][1] += 1 #Muutuja väärtus on enda väärtus +1.

        if coords[i][1] > screenY: #Kui sinine auto jõuab lõppu, muudetakse auto algus kohta.
            skoor += 1 #Skoorile lisandub üks punkt igakord kui sinine auto jõuab lõppu.
    ...
    tekst = font.render("SKOOR: " + str(skoor), True, [255, 255, 255]) #Arvutab palju kokku skoori saadud on ja kuvab seda ekraanil.
    pygame.display.flip() #Ekraani värskendamine.
    screen.blit(bg, [0, 0]) #Joonistatakse taust uuesti.
    screen.blit(red, [295, 370]) #Joonistatakse punane auto uuesti.
